...
async function updateTabCount() {
  try {
    const response = await browser.runtime.sendMessage({ type: 'GET_TAB_COUNT' });
...
function updateUI() {
  ...
  if (elements.maxTabsValue) elements.maxTabsValue.textContent = settings.maxTabs;
  ...
  // Global limit
  if (elements.globalLimitToggle) elements.globalLimitToggle.checked = settings.globalLimit;
  updateGlobalHint();
  
  // Allowlist
  if (elements.allowlistToggle) elements.allowlistToggle.checked = settings.allowlistEnabled;
  if (elements.allowlistContainer) {
    elements.allowlistContainer.classList.toggle('enabled', settings.allowlistEnabled);
  }
  renderDomains();
...
function updateLockUI() {
  ...
  if (elements.lockIcon) elements.lockIcon.style.display = isLocked ? 'block' : 'none';
  if (elements.unlockIcon) elements.unlockIcon.style.display = isLocked ? 'none' : 'block';
  if (elements.lockBtn) {
...
function updatePausePlayIcon() {
  ...
  if (elements.pauseIcon) elements.pauseIcon.style.display = isEnabled ? 'block' : 'none';
  elements.pauseBars.forEach(bar => bar.style.display = isEnabled ? 'block' : 'none');
  if (elements.playIcon) elements.playIcon.style.display = isEnabled ? 'none' : 'block';
  if (elements.disableBtn) {
    elements.disableBtn.title = isEnabled ? 'Disable protection' : 'Enable protection';
  }
}
...
function updateGlobalHint() {
  if (elements.globalHint) {
    ...
    if (perWindowHint) perWindowHint.style.display = settings.globalLimit ? 'none' : 'inline';
    if (globalHint) globalHint.style.display = settings.globalLimit ? 'inline' : 'none';
  }
}
...
async function addDomain() {
  const input = elements.allowlistInput.value.trim().toLowerCase();
  if (!input) return;
...
function setupEventListeners() {
  ...
  // Add domain
  if (elements.addDomainBtn) elements.addDomainBtn.addEventListener('click', addDomain);
  ...
  // Modal
  if (elements.modalCancel) elements.modalCancel.addEventListener('click', hideFrictionModal);
  if (elements.modalConfirm) elements.modalConfirm.addEventListener('click', confirmFrictionAction);
